...
window.customConfirm = (message, title = 'ยืนยันการดำเนินการ', type = 'warning') => {
    return new Promise((resolve) => {
        ...
        document.getElementById('customConfirmMessage').innerHTML = escapeHtml(message).replace(/\n/g, '<br>');
...
window.customPrompt = (message, title = 'กรอกข้อมูล', placeholder = '') => {
    return new Promise((resolve) => {
        ...
        document.getElementById('customPromptMessage').innerHTML = escapeHtml(message).replace(/\n/g, '<br>');
...
function handlePostLogin(user) {
    ...
    if (user.must_change_password) {
        ...
        customAlert("กรุณาเปลี่ยนรหัสผ่านก่อนเริ่มใช้งาน", "warning");
        return;
    }
...
document.getElementById("changePasswordForm").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (res.ok) { 
            ...
            if (me.ok) handlePostLogin(await me.json());
        } else { 
